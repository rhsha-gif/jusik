...
def test_scrub_redacts_credential_env_values_and_known_shapes() -> None:
    ...
    for leaked in ("Bearer abcdefghijklmnopqrstuvwxyz0123", "sk-abcdef", "hooks.slack.com/services", "0123456789abcdef0123456789abcdef", "통째로"):
...
def test_slack_post_scrubs_before_sending(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_send(url: str, body: bytes, timeout_s: float) -> tuple[int, str]:
        sent["url"] = url
