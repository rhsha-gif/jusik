...
from quantpilot.packages.core.normalization import symbol_key, unique_text
...
@dataclass(frozen=True)
class _BatchCheckFacts:
    exposure: BatchPortfolioExposure
    oversold_symbols: list[str]
    stale_input_reasons: list[str]
    candidate_policy_versions: list[int]
    idempotency_keys: list[str]
    seen_idempotency_keys: set[str]
    batch_notional: float
    has_buy: bool
    order_types_allowed: bool
...
def _stale_input_reasons(
    *,
    ...
    reasons.extend(f"quote_stale:{symbol}" for symbol in _unique(stale_quote_symbols))
    return _unique(reasons)


def _order_types_allowed(policy: UserPolicy, candidates: Sequence[_Candidate]) -> bool:
    allowed = all(candidate.intent.order_type in policy.allowed_order_types for candidate in candidates)
    if any(candidate.intent.order_type == OrderType.market for candidate in candidates) and not market_orders_enabled():
        return False
    return allowed


def _batch_check_facts(
    *,
    policy: UserPolicy,
    snapshot: PortfolioSnapshot,
    candidates: Sequence[_Candidate],
    ...
    return _BatchCheckFacts(
        exposure=exposure,
        oversold_symbols=oversold_symbols,
        stale_input_reasons=_stale_input_reasons(
            snapshot=snapshot,
        ...
        seen_idempotency_keys=set(seen_idempotency_keys).union(guardrail_state.submitted_idempotency_keys),
        ...
        order_types_allowed=_order_types_allowed(policy, candidates),
        max_position_ok=max_position_ok,
        max_sector_ok=max_sector_ok,
    )


def _append_check(name: str, condition: bool, *, passed: list[str], failed: list[str]) -> None:
    if condition:
        passed.append(name)
    else:
        failed.append(name)
...
def _evaluate_batch_checks(
    *,
    ...
    guardrail_state: GuardrailState,
    facts: _BatchCheckFacts,
) -> tuple[list[str], list[str]]:
    passed: list[str] = []
    failed: list[str] = []

    def check(name: str, condition: bool) -> None:
        _append_check(name, condition, passed=passed, failed=failed)
    ...
    check("policy_version_match", portfolio_plan.policy_version == policy.version and all(version == policy.version for version in facts.candidate_policy_versions))
    ...
    check("snapshot_not_stale", "snapshot_stale" not in facts.stale_input_reasons)
    check("quotes_not_stale", not any(reason.startswith("quote_stale:") for reason in facts.stale_input_reasons))
    check("quotes_available", all(_quote_input_available(candidate.intent, quotes) for candidate in candidates))
    check("available_cash_after_batch", facts.exposure.cash >= 0)
    check("min_cash_after_batch", facts.exposure.cash >= policy.min_cash_weight * snapshot.equity)
    check("max_position_weight_after_batch", facts.max_position_ok)
    check("max_concentration_weight_after_batch", facts.max_position_ok)
    check("max_sector_weight_after_batch", facts.max_sector_ok)
    check("no_short_sell_after_batch", not facts.oversold_symbols)
    check("max_daily_orders_after_batch", guardrail_state.daily_order_count + len(candidates) <= policy.max_daily_orders)
    check("max_daily_turnover_after_batch", guardrail_state.daily_turnover_used + facts.batch_notional <= policy.max_daily_turnover)
    check("order_type_allowed", facts.order_types_allowed)
    check("idempotency_keys_not_seen", len(facts.idempotency_keys) == len(set(facts.idempotency_keys)) and not any(key in facts.seen_idempotency_keys for key in facts.idempotency_keys))
    check("daily_loss_limit_not_triggered", not (facts.has_buy and snapshot.daily_loss_ratio <= policy.daily_loss_limit))
    check("monthly_loss_pause_new_buys", not (facts.has_buy and snapshot.monthly_loss_ratio <= policy.monthly_loss_pause_new_buys))
...
def _evaluate_candidates(
    *,
    ...
    passed, failed = _evaluate_batch_checks(
        policy=policy,
    ...
    return _BatchEvaluation(
        ...
        stale_input_reasons=facts.stale_input_reasons,
        portfolio_after_batch=facts.exposure,
    )
...
def _reject_full_batch(
    *,
    policy: UserPolicy,
    candidates: Sequence[_Candidate],
    evaluation: _BatchEvaluation,
) -> BatchRiskDecision:
    reasons = _decision_reasons(evaluation)
...
def _partial_batch_decision(
    *,
    ...
    seen_idempotency_keys: set[str],
    now: datetime,
    full_evaluation: _BatchEvaluation,
) -> BatchRiskDecision:
    accepted: list[_Candidate] = []
    rejected: list[_Candidate] = []
    rejected_reasons: dict[str, list[str]] = {}
    for candidate in candidates:
        trial_evaluation = _evaluate_batch(
            policy=policy,
            portfolio_plan=portfolio_plan,
            snapshot=snapshot,
    ...
    accepted_evaluation = _evaluate_batch(
        policy=policy,
        portfolio_plan=portfolio_plan,
        snapshot=snapshot,
        candidates=accepted,
...
def run_batch_risk_gate(
    *,
    ...
    full_evaluation = _evaluate_batch(
        policy=policy,
        portfolio_plan=portfolio_plan,
        snapshot=snapshot,
        candidates=candidates,
    ...
    if not risk_config.partial_allow:
        return _reject_full_batch(
            policy=policy,
            candidates=candidates,
            evaluation=full_evaluation,
        )

    return _partial_batch_decision(
        policy=policy,
        portfolio_plan=portfolio_plan,
        snapshot=snapshot,
        candidates=candidates,
