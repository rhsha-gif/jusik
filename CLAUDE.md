...
- `/vault-consult`: 지식 vault(`quantpilot-foundation/`) 조회와 인용 규약. 퀀트·리스크·집행·
  ...
  답할 때는 그 사실을 밝힌다.
- 리서치 에이전트(시황 3·투자 4·전략가 5·설계자 4, `qp-*`)와 잡 `run_market_brief`·`run_invest_research`·
  `run_macro_outlook`·`run_strategy_design`: `docs/research_agents.md`. 산출물은 읽기 전용이며 거래 입력이 아니다.
  전략가·설계자 런북은 `/qp-strategist`·`/qp-designer`; 설계자 레시피는 `quantpilot/docs/strategy_specs/`에 draft로만 놓인다.
