...
Apply this procedure only to an existing active board or a board the user asked to create. Aorch owns whether a board or delegation is needed. This skill does not authorize commits, push or integration. If a transition requires a commit that the user has not requested, record pending/uncommitted and stop before that transition. Preserve leases, scoped ownership and safety review.
...
<!-- aorch-generated: skill:workboard-flow; mode=native; edit .agents/aorch/definitions.json -->
