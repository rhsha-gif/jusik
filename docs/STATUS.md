...
`reconcile`은 메모리 사본에서 조회 결과를 미리 검증하며, `--apply`는 일시정지·계좌 귀속·
프로세스 잠금·원장 백업을 확인한 뒤 기존 누적 체결 경로로 복구한다. 복구 전용 전송기는
인증·잔고·일별 주문 조회만 허용한다. 중단 후 재시도에서도 체결·현금·비용·정정 보고가 중복되지 않는다.
평가 근거와 직전 거래일 기준이 불완전한 손익은 숨기고, 과거 그래프 표본은 보존하면서 표시에서 제외한다.

최종 검증 **1,632 passed·2 skipped**, smoke·`tach check` 및 독립 후속 검토 2건 PASS.
승인된 실제 모의 원장 복구와 재적용 불변성, 운용기·AI 작업기·발송기 생존, Slack 정정 보고 API 접수,
대시보드 반영을 확인했다. 신규 진입은 `paused`, 장후 잔량은 기존 지정가 청산 규칙의 다음 세션 대기다.
재가동 후 일시적 잔고 통신 오류는 자동 재조회로 회복했다. 다음 예정 거래일인 9월 14일부터
후속 인수 자동화가 실제 청산 증거를 확인한다. 청산 완료는 아직 확인하지 않았다.
상세 근거는 [복구 기록](plans/2026-09-11-paper-recovery.md), 개인 거래 자료는 저장소 밖에 보존한다.
...
## 최근 완료 (2026-08-12, 개인 투자 판단 절차 확립)

> 개인 재무 정보(금액·소득·잔고·보유)는 이 공개 저장소에 기록하지 않는다.
> 실제 판단 원장과 모니터는 비공개 저장소로 분리했고, 여기에는 **방법론만** 남긴다.

- **예측을 사실로 오독하는 실패를 절차로 막았다** — 이전 감사는 "수요가 옮겨갈 것"이라는
  미래 예측을 현재 자금 흐름으로 "실시간 반증" 판정했다. 예측은 현재 관측으로 기각할 수
  없고 현재 흐름은 기저율일 뿐이다. 이후 모든 테제는 `사실`과 `예측`을 분리해 적고,
  예측마다 **질문·시한·판정 출처** 3종을 갖추지 못하면 진행하지 않는다
  (근거: [[예측과 캘리브레이션]], Superforecasting 질문 계약 `SUPER2015-C03-001`).
- **조사 구조: 상호 비공유 독립 에이전트** — 촉매 조사와 반증(레드팀)을 서로 결과를 모르는
  에이전트로 병렬 수집했다. 레드팀이 스스로 사용 불가 논거 3건을 기각한 것은 좋았으나
  **구조가 아니라 우연**이었으므로, 후속 절차에서 논거별 독립 검증과 출처 등급 하한
  (C등급 단독 논거는 `미확인`으로 강등)을 의무화했다.
- **중첩 표본의 한계를 보고 규격에 못 박았다** — 가격 이력 전수 윈도우 시뮬레이션은
  윈도우가 강하게 겹쳐 사실상 사이클 2~3개의 반복 관측이다. 산출 확률은 **기저율 참고치로만
  제시하고 신뢰구간으로 쓰지 않는다.**
- **판단 이력의 덮어쓰기를 금지했다** — 보고서를 갱신하면서 이전 판정이 사라지는 문제를
  확인했다. 볼트의 실패 조건 *"사후확률로 과거 forecast를 덮어씀"*에 해당하므로, 판단
  기록은 append-only 원장으로 옮기고 변경 시 **수정 대신 `supersedes`를 건 새 레코드**를
  만든다.
- **비차단 한계 (라이브 준비 주장 아님)**: ① 이 작업은 QuantPilot 제품 경로와 무관한 개인
  연구이며 모니터는 **조회 전용으로 주문 권한이 없다** ② 시장 수치 상당수가 언론 2차
  인용(C등급)이라 실행 결정 전 원자료 재확인이 필요하다.
