...
**MCP가 없을 때의 폴백**: `vault` 도구가 목록에 없으면 현재 실행 환경에서 승인된 로컬 검색·파일 읽기 도구로 볼트를 검색하고
`홈.md` → `주제 MOC/` → `개요 — <책>` 순으로 탐색한다. 폴백을 썼다는 사실을 답변에
밝힌다 (검색 품질이 다르므로 놓쳤을 가능성이 있다). 볼트의 실제 경로는 호출자가 제공하거나 승인된 로컬 탐색으로 확인하며 추정하지 않는다. 셸 금지 역할은 셸 기반 검색을 쓰지 않는다. 읽기 도구와 실제 볼트 경로가 모두 없으면 호출자에게 실제 검색·정독 증거(질의·조회 시점·반환 경로·제목·읽은 본문·근거 위치)를 요청한다. 제공받은 조회는 호출자의 조회로 표시하고 직접 도구를 썼다고 주장하지 않는다. 증거 없이 볼트 인용을 만들어 내지 않는다.
...
Claude와 Codex에서 같은 검색·정독·인용 규율을 적용한다. 도구 이름의 접두사는 환경마다 다를 수 있으므로 실제 노출된 기능을 선택한다. 읽기 전용 역할에서는 아래 기록·편집·재색인 권한이 생기지 않는다. 필요한 변경은 권한을 가진 호출자에게 전달한다. MCP가 없다는 이유로 인증 설정을 읽거나 복사하지 않고, 로컬 임베딩 원칙을 원격 API로 바꾸지 않는다.
...
<!-- aorch-generated: skill:vault-consult; mode=native; edit .agents/aorch/definitions.json -->
